# Challenge Selecting DOM Elements

### Challenge tasks

1. Select the element with the ID `about`. Store the element in the variable `about`.
2. Select all the `<h2>` elements in the document. Set the color of the `<h2>` elements to a different color.
3. Select all elements with the class `.card`. Set their background color to the color of your choice.
4. Select only the first `<ul>` in the document. Assign it to a variable named `ul`.
5. Select only the second element with the class `.container`. Assign it to a variable named `container`.
...
card[i].style.backgroundColor = "lightgrey";
}
```
...
titleLink[i].style.color = "lightblue";
}
```
