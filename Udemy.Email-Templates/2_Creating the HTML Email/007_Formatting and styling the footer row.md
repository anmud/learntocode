...
Here we also first add some styling on the `<td>` tag. Inside this `<td>` we'll have content: some sample text you might have inside of your message; some links to "privacy" and "unsubscribe" with inline styles. 
...
Inside of the `footer row` we have `<td>` with the background color set the same as the `table` that centeres our entire email, which is also the same color we put into the `body background`. 

![footer-part](../footer-part.png)
