...
As always we start with adding some styling to `<td>`. then inside the `<td>` tag we'll add a `table` with the `image`, `some text`, and an `anchor tag`. We'll have four `tables` inside - each for the separate `individual callout`. Each `table` has a `width="135"`, aligned left.
...
![callouts-part](../callouts-part.png)

Later, we gonna be floating some of these `tables` for `browsers` that have more advanced `css support`. And all we need to do is to add a `break` tag so that we can clear some of the floats. 

After the fourth `table callout` let's set `break` tags: `<br style="clear: both;">`. Tag with `clear float` css definitions, when we float a table later, this will ensure that the outer most container, which is the purple row of promos, will extent high in the two .. of all of the table inside. 
...
<table class="callout_4" width="135" align="left" border="0" cellspacing="0" cellpadding="0">
						...
						<br style="clear: both;"><!--add break tag here-->
```