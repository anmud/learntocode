...
Now we gonna create a `call-to-action button` , it goes inside of the `content` and aligns over to the right hand side. It's gonna be set inside of the `table` and we gonna align that overall `table` to the right, so the `content` can flow to the left hand side. 

Let's find our row that holds the `content` and add some space before the `sample text`. We need to add our `button` before this text since we wanna align it to the right, we wanna to have the top of the `text` be as high as the top of the `button`. So, let's create a `table` before the `text`. Inside the `table` we'll have a `<td>` tag also with styling. 

Inside of the `<td>` we'll add an `anchor` tag, with some inline styling. 
### HTML
```html
                           ...
                           <table width="160" align="right" class="button" style="margin: 0px 0px 10px 30px; ">
								 <tr>
									 <td style="text-align: center; background-color: #71a412; padding: 10px 15px; border-radius: 5px;"> 
                                         <a href="#" style="color: #fff; font-size: 18px; letter-spacing: 1px; 
									   text-decoration: none; text-shadow: 0px 2px 2px #3a5606; font-family: Arial, Helvetica, sans-serif;"> 
									   Learn more</a> <!--add anchor tag-->
									 </td>
								 </tr>
							 </table><!--add table for the button-->
...
```
![call-to-actio-button](../call-to-action-button.png)

Let's also change the `links` in our `callouts`: for every callout we'll add some inline styling - `style="color: #efe1b0"`. And we'll have the yellow color for the individual callouts. 

![callout-link-color](../callout-link-color.png)

And finally we have the `email template` taht will work in  `desktop clients` and `desktop browsers`. 